...
from .model_architecture import TimesNet
...
def train_model(model, train_loader, val_loader, optimizer, loss_criterion):
    for epoch in range(EPOCHS):
        ...
        for batch_x, batch_y in train_loader:
            optimizer.zero_grad()
            x_mark_enc = torch.ones(batch_x.shape[0], batch_x.shape[1])
            logits = model(batch_x, x_mark_enc, None, None) # x_dec, x_mark_dec are None for classification
        ...
        with torch.no_grad():
            for val_x, val_y in val_loader:
                x_mark_enc = torch.ones(val_x.shape[0], val_x.shape[1])
                val_preds = model(val_x, x_mark_enc, None, None)
...
def evaluate_model(model, test_loader):
    ...
    with torch.no_grad():
        for logits, targets in test_loader:

            x_mark_enc = torch.ones(logits.shape[0], logits.shape[1])
            logits = model(logits, x_mark_enc, None, None)
...
if __name__ == "__main__":
    ...
    configs = Configs(
        ...
        top_k = 2,
        d_ff = MODEL_DIMENSION * 2,
        num_kernels=NUM_KERNELS,
    )
    model = TimesNet(configs=configs)
